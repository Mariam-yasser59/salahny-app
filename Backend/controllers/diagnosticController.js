...
const REQUIRED_FEATURES = [
  ...
  'ENGINE_RUN_TIME', 'ENGINE_LOAD', 'THROTTLE', 'TIMING_ADVANCE',
  'SHORT_TERM_FUEL_TRIM_BANK_1', 'LONG_TERM_FUEL_TRIM_BANK_1', 'FUEL_TANK',
  'FUEL_AIR_COMMANDED_EQUIV_RATIO', 'INTAKE_MANIFOLD_PRESSURE', 'INTAKE_AIR_TEMP',
  'ABSOLUTE_BAROMETRIC_PRESSURE', 'ABSOLUTE_THROTTLE_B', 'RELATIVE_THROTTLE_POSITION',
...
const requiredFeatureCount = (readings) =>
  REQUIRED_FEATURES.filter((key) => key in readings).length;
...
const FAULT_RECOMMENDATIONS = {
  'Engine Overheating': ['Check coolant level and radiator fan.', 'Inspect the thermostat.', 'Stop driving if warning light appears.'],
  'Alternator Failure': ['Test battery voltage (should be 13.8–14.4V).', 'Replace the alternator.', 'Check the drive belt.'],
  'Transmission Slip': ['Check transmission fluid level.', 'Inspect transmission bands.', 'Schedule transmission service.'],
  'Throttle Body Fault': ['Clean the throttle body.', 'Check throttle position sensor.', 'Inspect idle air control valve.'],
  'O2 Sensor Failure': ['Replace the oxygen sensor.', 'Check fuel mixture and injectors.', 'Inspect exhaust system for leaks.'],
  'Thermostat Stuck Open': ['Replace the thermostat.', 'Check coolant temperature sensor.', 'Allow engine to reach operating temperature.'],
};
...
const buildRuleBasedPrediction = (sensorReadings, faultCodes, mlError) => {
  ...
  const stTrim   = normalizedReading(sensorReadings, ['SHORT_TERM_FUEL_TRIM_BANK_1', 'short_term_fuel_trim_bank_1']);
  const ltTrim   = normalizedReading(sensorReadings, ['LONG_TERM_FUEL_TRIM_BANK_1', 'long_term_fuel_trim_bank_1']);
  const lambda   = normalizedReading(sensorReadings, ['FUEL_AIR_COMMANDED_EQUIV_RATIO', 'fuel_air_commanded_equiv_ratio'], 1.0);
  const load     = normalizedReading(sensorReadings, ['ENGINE_LOAD', 'engine_load']);
  ...
  if (rpm >= 3500 && load >= 50 && speed < 45) detected.push('Transmission Slip');
  if (rpm >= 4500 && speed < 20)               detected.push('Throttle Body Fault');
  if (stTrim >= 18 || ltTrim >= 10 || lambda < 0.94) detected.push('O2 Sensor Failure');
  ...
  const recommendations = fault
    ? [...(FAULT_RECOMMENDATIONS[fault] ?? []), ...(faultCodes.length > 0 ? ['Verify submitted OBD codes with a workshop.'] : [])]
  ...
  return {
    ...
    aiPrediction: {
      hasFault: !!fault,
      issue: summary,
      confidence: riskLevel === 'critical' ? 0.72 : riskLevel === 'warning' ? 0.60 : 0.5,
      urgency: riskLevel,
      explanation: `Rule-based assessment: coolant ${coolant}°C, RPM ${rpm}, speed ${speed} km/h, voltage ${voltage}V, ST fuel trim ${stTrim}%, LT fuel trim ${ltTrim}%, lambda ${lambda}.`,
...
const createDiagnostic = async ({
  ...
  if (sourceType === 'manual') {
    // Manual input must supply all features — no fallback allowed.
    ...
    // File uploads only need the 3 core signals; the ML service fills the rest with medians.
    const corePresent = ['ENGINE_RPM', 'COOLANT_TEMPERATURE', 'VEHICLE_SPEED'].filter((key) => key in readings).length;
    if (corePresent < 3) {
      throw Object.assign(new Error('OBD file must contain at least ENGINE_RPM, COOLANT_TEMPERATURE, and VEHICLE_SPEED'), { statusCode: 400 });
    }
  }
  let template;
  let status = 'success';
  ...
  const diagnostic = await Diagnostic.create({
    ...
    faultCodes: faultCodes.map((code) => {
      ...
      return { code, description: dtc.description, level: dtc.severity };
    }),
