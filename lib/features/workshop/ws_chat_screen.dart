...
class _WsChatScreenState extends State<WsChatScreen> {
  ...
  void initState() {
    super.initState();
    final bookings = AppData.i.workshopBookings;
    final matches = bookings.where((b) => b.id == widget.bookingId);
    _booking = matches.isNotEmpty
        ? matches.first
        : bookings.isNotEmpty
        ? bookings.first
        : null;
    RealtimeService.instance.joinBooking(widget.bookingId);
  ...
  Future<void> _loadContext() async {
    final contextData = await AppErrorHandler.guard<BookingChatContext>(
      context,
      () => _chatService.getBookingContext(widget.bookingId),
      fallbackMessage: 'Could not load customer contact details.',
    );
    if (!mounted || contextData == null) return;
    setState(() => _peerPhone = contextData.peerPhone);
  }
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Request #${_booking?.id ?? widget.bookingId} - ${_booking?.serviceName ?? 'Service'}',
