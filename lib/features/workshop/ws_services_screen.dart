...
class _WsServicesScreenState extends State<WsServicesScreen> {
  ...
  Future<void> _load() async {
    ...
    final data = await AppErrorHandler.guard<List<WsServiceItem>>(
      context,
      _service.getServices,
      fallbackMessage: 'Could not load workshop services.',
    );
    if (!mounted) return;
    setState(() {
      _services = data ?? AppData.i.workshopServices;
      _loading = false;
    });
  }
  ...
  Future<void> _deleteService(int index) async {
    ...
    final ok = await AppErrorHandler.guard<bool>(
      context,
      () async {
        await _service.deleteService(item.id.isEmpty ? item.name : item.id);
        return true;
      },
      fallbackMessage: 'Could not remove the service.',
    );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _AddServiceSheet(
        onAdd: (s) async {
          ...
          final added = await AppErrorHandler.guard<WsServiceItem>(
            context,
            () => _service.addService(s),
            fallbackMessage: 'Could not add the service.',
          );
          if (!mounted || added == null) return;
          setState(() => _services.add(added));
        },
      ),
    );
  }
...
class _AddServiceSheet extends StatefulWidget {
  ...
  @override State<_AddServiceSheet> createState() => _AddServiceSheetState();
}
...
class _AddServiceSheetState extends State<_AddServiceSheet> {
  ...
  String _emoji = 'Service';
  static const _emojis = ['Service', 'Oil', 'Tire', 'Battery', 'Wash', 'AC', 'Engine', 'Scan', 'Brake'];

  @override void dispose() { _nameCtrl.dispose(); _priceCtrl.dispose(); _durCtrl.dispose(); super.dispose(); }
            ...
            child: ListView(
              ...
              children: _emojis.map((e) => GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Center(child: Text(e, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AC.t1))),
                    ),
                  )).toList(),
            ),
          ),
