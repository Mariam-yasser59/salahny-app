...
class _WsDiagnosticsScreenState extends State<WsDiagnosticsScreen>
  ...
  void dispose() {
    _scanAnim.dispose();
    _rpm.dispose(); _coolant.dispose(); _speed.dispose();
    _voltage.dispose(); _runTime.dispose(); _engineLoad.dispose();
    _throttle.dispose(); _timingAdvance.dispose();
    _stFuelTrim.dispose(); _ltFuelTrim.dispose();
    _fuelTank.dispose(); _fuelAirRatio.dispose();
    _intakeManifoldPressure.dispose(); _intakeAirTemp.dispose();
    _barometricPressure.dispose(); _absThrottleB.dispose();
    _relThrottlePos.dispose(); _pedalD.dispose(); _pedalE.dispose();
    _commandedThrottle.dispose(); _catTempS1.dispose(); _catTempS2.dispose();
    _evapPurge.dispose(); _codesCtrl.dispose();
    super.dispose();
  }
  ...
  Map<String, double> _buildSensorReadings() => {
    ...
    'INTAKE_MANIFOLD_PRESSURE':          double.parse(_intakeManifoldPressure.text.trim()),
    'INTAKE_AIR_TEMP':                   double.parse(_intakeAirTemp.text.trim()),
    'ABSOLUTE_BAROMETRIC_PRESSURE':      double.parse(_barometricPressure.text.trim()),
  ...
  Future<void> _uploadFile() async {
    if (widget.linkedRequestId == null || widget.linkedRequestId!.isEmpty) {
      AppErrorHandler.showMessage(context, 'Open diagnostics from a booking first.');
      return;
    }
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            if (_source == 0)
              _UploadPanel(onTap: _uploadFile).animate().fadeIn(duration: 300.ms),
            if (_source == 1)
              _ManualPanel(
                rpm: _rpm, coolant: _coolant, speed: _speed,
                voltage: _voltage, runTime: _runTime,
                engineLoad: _engineLoad, throttle: _throttle,
                timingAdvance: _timingAdvance, stFuelTrim: _stFuelTrim,
                ltFuelTrim: _ltFuelTrim, fuelTank: _fuelTank,
                ...
                pedalD: _pedalD, pedalE: _pedalE,
                commandedThrottle: _commandedThrottle,
                catTempS1: _catTempS1, catTempS2: _catTempS2,
                evapPurge: _evapPurge, codesCtrl: _codesCtrl,
              ).animate().fadeIn(duration: 300.ms),
            if (_source >= 0) ...[
              const SizedBox(height: 28),
              if (_scanning)
                _ScanningOverlay(controller: _scanAnim)
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('AI-Powered Vehicle Diagnostics',
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AC.t1)),
              Text(
...
class _SourceGrid extends StatelessWidget {
  ...
  static const _sources = [
    ...
    (icon: Icons.edit_note_rounded,   label: 'Manual Entry',   sub: 'Enter all 23 OBD values',  color: AC.warning),
  ];
    ...
    children: _sources.asMap().entries.map((e) {
      ...
      return GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            gradient: sel
                ? LinearGradient(colors: [s.color.withValues(alpha: 0.18), AC.s2])
            ...
            boxShadow: sel
                ? [BoxShadow(color: s.color.withValues(alpha: 0.2), blurRadius: 14)]
                : null,
          ),
          child: Row(
            children: [
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        color: AC.s2,
        borderRadius: Rd.lgA,
        border: Border.all(color: AC.success.withValues(alpha: 0.3), width: 1.5),
      ),
      child: Column(
        children: [
          Container(
          ...
          const Text('Tap to Select OBD File',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AC.t1)),
          const SizedBox(height: 6),
          const Text('Supports .csv and .json OBD log formats',
              style: TextStyle(fontSize: 12, color: AC.t3), textAlign: TextAlign.center),
        ],
      ),
    ),
  );
...
class _ManualPanel extends StatelessWidget {
  ...
  final TextEditingController voltage, runTime, engineLoad, throttle, timingAdvance;
  final TextEditingController stFuelTrim, ltFuelTrim, fuelTank, fuelAirRatio;
  final TextEditingController intakeManifoldPressure, intakeAirTemp, barometricPressure;
  final TextEditingController absThrottleB, relThrottlePos, pedalD, pedalE, commandedThrottle;
  final TextEditingController catTempS1, catTempS2;
  final TextEditingController evapPurge, codesCtrl;

  const _ManualPanel({
    required this.rpm, required this.coolant, required this.speed,
    required this.voltage, required this.runTime, required this.engineLoad,
    required this.throttle, required this.timingAdvance,
    required this.stFuelTrim, required this.ltFuelTrim,
    required this.fuelTank, required this.fuelAirRatio,
    required this.intakeManifoldPressure, required this.intakeAirTemp,
    required this.barometricPressure, required this.absThrottleB,
    required this.relThrottlePos, required this.pedalD, required this.pedalE,
    required this.commandedThrottle, required this.catTempS1, required this.catTempS2,
    required this.evapPurge, required this.codesCtrl,
  });
  ...
  Widget build(BuildContext context) => WsCard(
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('OBD-II SENSOR INPUTS',
            style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                color: AC.t3, letterSpacing: 1.1)),
        ...
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Field('Control Module Voltage', 'e.g. 14.2', 'V', voltage)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Engine Run Time', 'e.g. 300', 's', runTime)),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _Field('Engine Load', 'e.g. 45', '%', engineLoad)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Throttle Position', 'e.g. 20', '%', throttle)),
        ]),
        const SizedBox(height: 12),
        ...
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Field('Short-Term Fuel Trim B1', 'e.g. -2.3', '%', stFuelTrim)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Long-Term Fuel Trim B1', 'e.g. 1.5', '%', ltFuelTrim)),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _Field('Fuel Tank Level', 'e.g. 65', '%', fuelTank)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Fuel/Air Equiv Ratio', 'e.g. 1.00', 'λ', fuelAirRatio)),
        ]),
        const SizedBox(height: 16),
        ...
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Field('Intake Manifold Pressure', 'e.g. 101', 'kPa', intakeManifoldPressure)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Intake Air Temp', 'e.g. 30', '°C', intakeAirTemp)),
        ]),
        const SizedBox(height: 12),
        _Field('Absolute Barometric Pressure', 'e.g. 101', 'kPa', barometricPressure),
        const SizedBox(height: 16),

        _GroupLabel('Throttle & Pedal', AC.success),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Field('Absolute Throttle B', 'e.g. 18', '%', absThrottleB)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Relative Throttle Pos', 'e.g. 12', '%', relThrottlePos)),
        ]),
        const SizedBox(height: 12),
        Row(children: [
        ...
        const SizedBox(height: 12),
        _Field('Commanded Throttle Actuator', 'e.g. 20', '%', commandedThrottle),
        const SizedBox(height: 16),

        _GroupLabel('Catalyst Temperature', AC.error),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(child: _Field('Catalyst Temp B1S1', 'e.g. 520', '°C', catTempS1)),
          const SizedBox(width: 12),
          Expanded(child: _Field('Catalyst Temp B1S2', 'e.g. 490', '°C', catTempS2)),
        ]),
        const SizedBox(height: 16),

        _GroupLabel('Evaporative System', AC.success),
        const SizedBox(height: 10),
        _Field('Commanded Evaporative Purge', 'e.g. 50', '%', evapPurge, optional: true),
        const SizedBox(height: 16),
...
class _Field extends StatelessWidget {
  ...
  final bool optional;
  const _Field(this.label, this.hint, this.unit, this.ctrl, {this.optional = false});
  ...
  Widget build(BuildContext context) => AppField(
    ...
    suffix: unit.isNotEmpty
        ? Padding(
            ...
            child: Text(unit, style: const TextStyle(fontSize: 12, color: AC.t3)),
          )
        : null,
    validator: (value) {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(
      children: [
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: AC.redGrad,
                  shape: BoxShape.circle,
                  boxShadow: [BoxShadow(color: AC.red.withValues(alpha: 0.55), blurRadius: 30)],
                ),
        ...
        const SizedBox(height: 6),
        const Text('AI model is processing all 23 sensor readings',
