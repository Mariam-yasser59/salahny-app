...
import '../driver/booking/services/tracking_service.dart';
...
class _WsActiveJobsScreenState extends State<WsActiveJobsScreen> {
  ...
  Future<void> _toggleLiveTracking(_JobEntry entry) async {
    final current = _liveTracking.remove(entry.b.id);
    if (current != null) {
    ...
    if (!enabled) {
      if (mounted) {
        AppErrorHandler.showMessage(context, 'Enable location services first.');
      }
      return;
    }
    ...
    final stream =
        Geolocator.getPositionStream(
          ...
          _tracking.postUpdate(
            bookingId: entry.b.id,
            latitude: position.latitude,
            longitude: position.longitude,
            note: 'Live workshop location',
          );
        });
    _liveTracking[entry.b.id] = stream;
    if (mounted) setState(() {});
  }

  @override
  void dispose() {
    for (final subscription in _liveTracking.values) {
      subscription.cancel();
    }
    super.dispose();
  }
    ...
    body: _jobs.isEmpty
        ...
        : ListView.separated(
            ...
            itemBuilder: (_, i) => _JobCard(
              entry: _jobs[i],
              onComplete: _jobs[i].progress < 1.0
                  ? () => _markComplete(_jobs[i])
                  : null,
              isLive: _liveTracking.containsKey(_jobs[i].b.id),
              onToggleLive: () => _toggleLiveTracking(_jobs[i]),
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(
                  done ? Icons.check_rounded : Icons.build_rounded,
                  color: Colors.white,
                  size: 22,
                ),
              ),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text(
                  b.customerName,
                  style: const TextStyle(fontSize: 12, color: AC.t2),
                ),
          ...
          Row(
            children: [
              Expanded(
                child: WsBtn(
                  label: isLive ? 'Stop Live GPS' : 'Start Live GPS',
                  outline: true,
                  icon: isLive
