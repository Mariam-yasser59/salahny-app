...
class ServiceApi {
  ...
  Future<List<ServiceModel>> getServices() async {
    final response = await _client.get('/services', auth: false) as Map<String, dynamic>;
    final items = response['data'] as List<dynamic>? ?? const [];
    final services = items
        .map((item) => _mapService(item as Map<String, dynamic>))
        .toList(growable: false);
    MockData.setRemoteServices(services);
    return services;
  }

  Future<List<PackageModel>> getPackages() async {
    final response = await _client.get('/packages', auth: false) as Map<String, dynamic>;
    final items = response['data'] as List<dynamic>? ?? const [];
    final packages = items
        .map((item) => _mapPackage(item as Map<String, dynamic>))
        .toList(growable: false);
    MockData.setRemotePackages(packages);
    return packages;
  }

  Future<Map<String, dynamic>> createRequest(String endpoint, Map<String, dynamic> payload) async {
