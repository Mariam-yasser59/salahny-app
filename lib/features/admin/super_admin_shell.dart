...
class _SuperAdminShellState extends State<SuperAdminShell> {
  ...
  Future<void> _sync() async {
    ...
    final ok = await AppErrorHandler.guard<bool>(context, () async {
      ...
      return true;
    }, fallbackMessage: 'Could not load admin data right now.');
    if (!mounted) return;
    setState(() {
      _loading = false;
      if (ok != true) {
        _loadError = 'Could not load admin data right now.';
      }
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: _index == 0
            ? [
                ...
                IconButton(
                  onPressed: () => Navigator.pushNamed(context, R.saDocuments),
                  icon: const Icon(Icons.verified_user_rounded, color: AC.t2),
                  tooltip: 'Documents',
                ),
              ]
            : _index == 4
