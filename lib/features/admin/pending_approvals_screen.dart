...
class _PendingApprovalsViewState extends State<PendingApprovalsView> {
  ...
  Future<void> _updateWorkshop(String id, AdminAccountStatus status) async {
    final ok = await AppErrorHandler.guard<bool>(context, () async {
      await _service.updateWorkshop(
        id: id,
        status: status,
        isVerified: status == AdminAccountStatus.active,
      );
      await _refresh();
      return true;
  ...
  Widget build(BuildContext context) {
    final pendingDrivers = AppCache.pendingDrivers;
    final pendingWorkshops = AppCache.pendingWorkshops;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_tab == 'Drivers') ...[
            ...pendingDrivers.map(
              (driver) => _DriverApprovalCard(
                driver: driver,
                onApprove: () =>
                    _updateDriver(driver.id, AdminAccountStatus.active),
                onReject: () =>
                    _updateDriver(driver.id, AdminAccountStatus.rejected),
              ),
            ),
          ] else ...[
            ...pendingWorkshops.map(
              (workshop) => _WorkshopApprovalCard(
                workshop: workshop,
                onApprove: () =>
                    _updateWorkshop(workshop.id, AdminAccountStatus.active),
                onReject: () =>
                    _updateWorkshop(workshop.id, AdminAccountStatus.rejected),
              ),
            ),
          ],
