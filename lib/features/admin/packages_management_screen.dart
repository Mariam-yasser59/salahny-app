...
class PackagesManagementScreen extends StatefulWidget {
  ...
  @override
  State<PackagesManagementScreen> createState() =>
...
class _PackagesManagementScreenState extends State<PackagesManagementScreen> {
  ...
  Widget build(BuildContext context) {
    final packages = AppCache.managedPackages;
      ...
      actions: [
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(color: AC.gold, borderRadius: Rd.mdA),
                  ...
                  children: [
                      ...
                      onTap: () => AppErrorHandler.guard<void>(
                        ...
                        () async {
                          await _service.updatePackage(
                            pkg.copyWith(isEnabled: !pkg.isEnabled),
                          );
                          await _refresh();
                        },
                        fallbackMessage:
                            'Could not update this package right now.',
                      ),
                    ),
                    ...
                    _miniAction(
                      ...
                      onTap: () => AppErrorHandler.guard<void>(
                        ...
                        fallbackMessage:
                            'Could not delete this package right now.',
                      ),
                    ),
                    const SizedBox(width: 8),
                    _miniAction(
                      label: 'Subscribers',
                      onTap: () => _showSubscribers(context, pkg.id),
                    ),
                  ],
                ),
              ],
  ...
  void _openEditor(BuildContext context, {ManagedPackage? pkg}) {
    ...
    final originalPriceCtrl = TextEditingController(
      text: pkg?.originalPrice.toStringAsFixed(0) ?? '',
    );
        ...
        children: [
          ...
          AppBtn(
            ...
            onTap: () {
              ...
              AppErrorHandler.guard<void>(context, () async {
                if (pkg == null) {
                  await _service.createPackage(next);
                } else {
                  await _service.updatePackage(next);
                }
                await _refresh();
                if (!context.mounted) return;
                Navigator.pop(context);
              }, fallbackMessage: 'Could not save this package right now.');
            },
          ),
        ],
      ),
  ...
  Future<void> _showSubscribers(BuildContext context, String packageId) async {
    final items = await AppErrorHandler.guard(
      context,
      () => _service.getPackageSubscribers(packageId),
      fallbackMessage: 'Could not load subscribers.',
    );
    if (!context.mounted || items == null) return;
    showAdminInfoDialog(
      context: context,
      title: 'Package Subscribers',
      child: Column(
        children: items.isEmpty
            ? const [
                Text('No subscribers yet.', style: TextStyle(color: AC.t3)),
              ]
            : items
                  .map(
                    (item) => ListTile(
