...
class AdminService {
  ...
  Future<Map<String, dynamic>> syncDashboard() async {
    ...
    AppCache.setRemoteAdminDrivers(drivers);
    AppCache.setRemoteAdminWorkshops(workshops);
    AppCache.setRemoteAdminBookings(bookings);
    AppCache.setRemoteManagedServices(services);
    AppCache.setRemoteManagedPackages(packages);
    AppCache.setRemoteActivityLogs(logs);
    AppCache.setRemoteAdminAnalytics(
      AdminAnalyticsData(
        bookingsByStatus:
            (analytics['bookingsByStatus'] as Map<String, dynamic>? ?? const {})
                .map(
                  (key, value) => MapEntry(key, (value as num?)?.toInt() ?? 0),
                ),
        activeSubscribers: (stats['activeSubscribers'] as num?)?.toInt() ?? 0,
        pendingDocuments: (stats['pendingDocuments'] as num?)?.toInt() ?? 0,
        revenueByMonth:
            (analytics['revenueByMonth'] as List<dynamic>? ?? const [])
                .cast<Map<String, dynamic>>()
                .map(
                  (item) => MonthlyRevenuePoint(
                ...
                .toList(growable: false),
        topWorkshops: (analytics['topWorkshops'] as List<dynamic>? ?? const [])
            .cast<Map<String, dynamic>>()
            .map(
              (item) => TopWorkshopPoint(
  ...
  Future<void> refreshDrivers() async {
    ...
    final items = ((response['data'] as List<dynamic>?) ?? const [])
        .map((item) => _mapDriver(item as Map<String, dynamic>))
        .toList(growable: false);
    AppCache.setRemoteAdminDrivers(items);
  }
  ...
  Future<void> refreshWorkshops() async {
    ...
    final items = ((response['data'] as List<dynamic>?) ?? const [])
        .map((item) => _mapWorkshop(item as Map<String, dynamic>))
        .toList(growable: false);
    AppCache.setRemoteAdminWorkshops(items);
  }
  ...
  Future<void> refreshBookings() async {
    final response =
        await _client.get('/admin/bookings') as Map<String, dynamic>;
    final items = ((response['data'] as List<dynamic>?) ?? const [])
        .map((item) => _mapBooking(item as Map<String, dynamic>))
        .toList(growable: false);
    AppCache.setRemoteAdminBookings(items);
  }
  ...
  Future<void> refreshLogs() async {
    ...
    final items = ((response['data'] as List<dynamic>?) ?? const [])
        .map((item) => _mapLog(item as Map<String, dynamic>))
        .toList(growable: false);
    AppCache.setRemoteActivityLogs(items);
  }

  Future<List<Map<String, dynamic>>> getBookingChats() async {
    final response =
        await _client.get('/admin/chats/bookings') as Map<String, dynamic>;
  ...
  Future<List<Map<String, dynamic>>> getEmergencyRequests() async {
    final response =
        await _client.get('/admin/emergency') as Map<String, dynamic>;
    return (response['data'] as List<dynamic>? ?? const [])
        .cast<Map<String, dynamic>>();
  }

  Future<void> assignEmergencyWorkshop(
    String requestId,
    String workshopId,
  ) async {
    await _client.patch('/admin/emergency/$requestId/assign-workshop', {
      'workshopId': workshopId,
    });
  }

  Future<List<Map<String, dynamic>>> getAdminDiagnostics() async {
    final response =
        await _client.get('/admin/diagnostics') as Map<String, dynamic>;
    return (response['data'] as List<dynamic>? ?? const [])
        .cast<Map<String, dynamic>>();
  }

  Future<void> updateUserStatus(String id, AdminAccountStatus status) async {
    await _client.patch('/admin/users/$id/status', {
      'status': _accountStatusValue(status),
    });
  }
  ...
  Future<List<Map<String, dynamic>>> getPackageSubscribers(String id) async {
    final response =
        await _client.get('/admin/packages/$id/subscribers')
