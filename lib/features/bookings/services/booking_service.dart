...
class BookingService {
  ...
  Future<List<BookingModel>> getBookings() async {
    ...
    MockData.setRemoteBookings(bookings);
    return bookings;
  }

  Future<BookingModel> createBooking(Map<String, dynamic> payload) async {
    final response = await _client.post('/bookings', payload) as Map<String, dynamic>;
    final data = response['data'] as Map<String, dynamic>? ?? response;
    final booking = _mapBooking(data);
    final current = MockData.bookings.toList(growable: true);
    current.insert(0, booking);
    MockData.setRemoteBookings(current);
    return booking;
  }

  Future<BookingModel> updateBookingStatus(String id, String status) async {
    final response =
        await _client.patch('/bookings/$id/status', {'status': status}) as Map<String, dynamic>;
    final data = response['data'] as Map<String, dynamic>? ?? response;
  ...
  BookingModel _mapBooking(Map<String, dynamic> json) {
    ...
    return BookingModel(
      ...
      date: date == null ? appShortDate() : appShortDate(date.difference(DateTime.now()).inDays),
