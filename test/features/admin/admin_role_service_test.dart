...
void main() {
  ...
  group('AdminService remote mapping', () {
    test('refreshWorkshops stores mapped workshop list from backend', () async {
      ...
      expect(MockData.adminWorkshops.first.name, 'Alex Motors');
      expect(MockData.adminWorkshops.first.status, AdminAccountStatus.active);
      expect(MockData.adminWorkshops.first.isVerified, isTrue);
    });
    ...
    test('refreshLogs stores mapped admin activity logs from backend', () async {
      ...
      expect(MockData.activityLogs.first.action, 'Package updated');
      expect(MockData.activityLogs.first.target, 'Premium Care');
    });
  });
}
