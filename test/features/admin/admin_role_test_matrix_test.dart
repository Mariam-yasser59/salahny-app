...
void main() {
  group('Test — Happy Path', () {
    ...
    test('admin can refresh workshop data from backend', () async {
      ...
      expect(AppCache.adminWorkshops.first.name, 'Prime Workshop');
  ...
  group('Test — Edge Cases', () {
    ...
    test('empty remote logs response does not crash admin sync', () async {
      ...
      expect(AppCache.activityLogs, isEmpty);
    });
  });
  ...
  group('Test — Negative Cases', () {
    ...
    test('malformed backend workshop payload falls back safely', () async {
      ...
      expect(AppCache.adminWorkshops, hasLength(1));
      expect(AppCache.adminWorkshops.first.name, '');
      expect(AppCache.adminWorkshops.first.status, AdminAccountStatus.active);
      expect(AppCache.adminWorkshops.first.specialty, 'Full Service');
    });
  });
}
