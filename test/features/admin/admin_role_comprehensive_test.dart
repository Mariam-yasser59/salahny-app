...
void main() {
  ...
  group('Admin credentials', () {
    ...
    test('rejects invalid hidden super admin credentials', () {
      expect(
        AppCache.validateAdminCredentials('admin@salahny.com', 'wrong'),
        isFalse,
      );
      expect(
  ...
  group('AdminService', () {
    test('syncDashboard maps dashboard payload into AppCache caches', () async {
      ...
      expect(AppCache.drivers, hasLength(1));
      expect(AppCache.adminWorkshops, hasLength(1));
      expect(AppCache.adminBookings, hasLength(1));
      expect(AppCache.managedServices, hasLength(1));
      expect(AppCache.managedPackages, hasLength(1));
      expect(AppCache.activityLogs, hasLength(1));
      expect(AppCache.adminBookings.first.status, AdminBookingStatus.active);
    });
    ...
    test('refreshDrivers stores mapped remote driver data', () async {
      ...
      expect(AppCache.drivers, hasLength(1));
      expect(AppCache.drivers.first.name, 'Remote Driver');
      expect(AppCache.drivers.first.status, AdminAccountStatus.pending);
    });
